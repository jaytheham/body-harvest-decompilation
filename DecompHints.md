...
- **GFX macros vs raw word writes**: Use proper GFX macros (`gSPPerspNormalize`, `gSPMatrix`, etc.) via `D_8005BB2C++` instead of raw `{ Gfx *gfx = D_8005BB2C; D_8005BB2C = gfx+1; gfx->words.w0 = ...; gfx->words.w1 = ...; }` blocks. The raw blocks introduce extra local variables that cause register allocation differences. **Jay's directive**: all GFX hex writes (e.g. `temp->unk0 = 0xE7000000`) must be decoded to named macros. Use `tools/gfxdis.sh -w <hi> <lo> ...` (bash wrapper, default F3DEX GBI). Docker required — install with `sudo apt-get install docker.io && sudo chmod 666 /var/run/docker.sock`. The tool auto-builds the `bh-gfxdis` image on first run from `tools/Dockerfile.gfxdis`. Pattern for each command: `gXXX(D_8005BB2C++, args...)` where `D_8005BB2C` is the display list pointer advanced with `++` post-increment.
