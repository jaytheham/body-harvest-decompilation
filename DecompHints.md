...
1. Fix the control flow to match the asm's ordering of basic blocks (it's very predictable, so this is simple; mips_to_c generally does it automatically).
2. Fix large reorderings.
3. Ensure the same set of saved registers are used (e.g. by making sure to reuse variables, duplicating expressions rather than putting them in variables, etc.).
4. Fix small reorderings.
5. Fix regalloc.
...
**Extern declarations vs local variables**: The way you declare global references affects the generated code. For globals with relocation symbols (e.g., `extern u8 D_80257A00[]`), the compiler generates relocatable code where `objdump` displays immediate values as `0x0000` before linking. This is normal and expected - the linker will fill in the actual addresses. Don't be alarmed if the hex values in generated assembly don't match the original until after the full link step.
...
**Variable declaration order affects register allocation**: The order in which local variables are declared significantly impacts which registers they receive. For example:
...
will allocate registers differently than:
...
This is one of the primary tools for fixing register allocation mismatches.

**Variable initialization order matters**: Not only declaration order, but the order in which variables are assigned/initialized affects codegen. Moving an initialization statement earlier or later can cause instruction reordering and regalloc changes. Use this strategically when trying to match exact register allocation.

**Address-of operator vs pointer casting**: When working with global integer variables that represent addresses, the method of obtaining the pointer matters:

- `ptr = &globalVar;` generates `lui+addiu` pattern (preferred)
- `ptr = (type *)globalVar;` may generate different patterns like `lui+lw`

Use `&symbol` when you want the address of a global, as it more closely matches expected assembly patterns.

**Breaking up complex expressions**: Expressions like `a1 + (t0 * a2) + 0x3A` in a single statement can cause suboptimal register allocation. Breaking into separate temporary variables can improve register allocation:

```c
t1 = (t0 * a2);
result = *(u16 *)(a1 + t1 + 0x3A);
```

instead of:

```c
result = *(u16 *)(a1 + (t0 * a2) + 0x3A);
```
