...
angle_done:
	...
	if (arg1->unkE != 0) {
		...
		if ((vehicle == D_80052B34) && (arg1->door2InteriorId < 0x32)) {
			...
			entry->unk10 = (f32)(entry->unk10 + ((((f32)coss(D_80052B34->unkE) / 32768.0) * ((f32)sp5C / 32768.0) * D_80052B34->unk58) + (f64)D_80052B34->unk30));
			entry->unk14 = (f32)(entry->unk14 + ((((f32)sins(D_80052B34->unk10) / 32768.0) * D_80052B34->unk58) + D_80052B34->unk34));
			sp5C = coss(D_80052B34->unk10);
			entry->unk18 = (f32)(entry->unk18 + ((((f32)sins(D_80052B34->unkE) / 32768.0) * ((f32)sp5C / 32768.0) * D_80052B34->unk58) + (f64)D_80052B34->unk38));
		}
	}
					...
					if (damage != 0) {
						...
						if (result != 8) {
							...
							if (vehicle->unk1A == 0) {
								...
								func_80102D00_111CB0(vehicle, (f32)(((f32)dx * pushScale) / 10.0), (f32)((f32)dy * pushScale), (f32)((f64)((f32)dz * pushScale) / 10.0));
							}
						}
					}
				}
...
void func_8012B26C_13A21C(void) {
	...
	for (i = D_8015F9E4 - 1; i >= 0; i--) {
		...
		ctrl = (UnkProjectileCtrl_8012B26C *)&projectile->unkC;
...
s32 func_8012E03C_13CFEC(s16 arg0, s16 arg1, s16 arg2)
{
	...
	while (i--)
	{
		if (D_8015FAD0[i].unk2C == 0)
			continue;
		if (a0 < ((D_8015FAD0[i].unk0 >> 16) - D_8015FAD0[i].unk18))
			continue;
		if ((D_8015FAD0[i].unk18 + (D_8015FAD0[i].unk0 >> 16)) < a0)
			continue;
		if (a1 < ((D_8015FAD0[i].unk4 >> 16) - D_8015FAD0[i].unk1C))
			continue;
		if ((D_8015FAD0[i].unk1C + (D_8015FAD0[i].unk4 >> 16)) < a1)
			continue;
		if (a2 < ((D_8015FAD0[i].unk8 >> 16) - D_8015FAD0[i].unk1A))
			continue;
		if ((D_8015FAD0[i].unk1A + (D_8015FAD0[i].unk8 >> 16)) < a2)
			continue;
		return i;
	}
