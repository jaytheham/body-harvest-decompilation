...
2. Orchestrate processing of these functions one at a time. For each function, you will create a new subagent, the subagent must use agentName `BH Decompiler`, and use it to decompile a single function (if the subagent gets rate-limited you can update `decomp.agent.md` to use a different model version (`Claude Sonnet 4.5 (copilot)` and `Claude Sonnet 4.6 (copilot)` are available)).
