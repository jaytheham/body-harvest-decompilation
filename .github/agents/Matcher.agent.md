...
Matching decompilation project for Body Harvest (N64).
Goal: Create C code that compiles to the exact same assembly as the original game ROM.
Your task: Modify an existing C function iteratively until it produces byte-for-byte identical assembly.
C89 code, compiler IDO 5.3 -O2 -mips2 -32.
...
- `build/`: Readonly - compiled object files and ROM image.
...
- Get the full assembly of a function after building by adding param `--show=target` or `--show=current` to the above diff command.
- You can see structural-only differences (skiping register & stack differences) in the diff output by adding param `--structural` to the above diff command.
...
2. Read `DecompHints.md` for general matching advice.
...
Prioritize incorrect, missing, and out-of-order instructions, ignore register allocation and stack placement until all logic is correct.
A change can produce more accurate logic, but alter register/stack allocation in a way that causes more differences overall, this is OK, the goal is to get the logic correct first, then optimize the register/stack allocation to match the target assembly.
...
Ignore git history, the current state is the best version of the code found so far.

- Declarations of global symbols must go in `include/variables.us.h`.
...
If you haven't matched the function after 20 attempts, revert the code to the version with the best  logic you found.

Think about whether there is some general pattern or insight in the successful changes you made, and if so update `/ExampleFixes` with new or updated notes to help future decomp.