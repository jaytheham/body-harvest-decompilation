...
5. Add definitions for these variables in the appropriate existing C file, make sure to use the correct types as determined by definitions in `variables.us.h` and load/store instructions in `.s` files, order, and initial values as in the original ROM. Define the variables before the first function in the C file.
