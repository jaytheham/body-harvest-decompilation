...
Not all functions in C are decompiled yet so you may need to search for variable references in the .s assembly files in `asm/nonmatchings` to find the correct C file.
...
6. Run the extract script to update the linker script, then run the build script to compile the ROM. You must do both after making yaml or code changes.
