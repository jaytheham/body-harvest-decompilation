...
- `docker exec -it bh-container bash -c "tools/asm-differ/diff.py --no-pager --compress-matching 3 func_8011CC20_12BBD0"` - Compare original and generated assembly for a specific function. Ignore instructions from beyond the function boundaries.
...
Rewrite C code to match the target assembly more closely.
Always try removing intermediate variables and simplifying before adding new ones.
Think about how a person would have originally written the code in C to produce the assembly you see, avoid writing C code that looks like it was generated by a decompiler e.g. goto statements, complex expressions, pointer arithmetic, etc. Use the tools mentioned earlier to search for patterns in the original assembly and see how other functions were written to achieve similar assembly output.
